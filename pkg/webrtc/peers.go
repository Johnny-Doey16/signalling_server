...
type Room struct {
	...
	Peers *Peers
	Hub   *chat.Hub
	User  map[string]*User // TODO: Move to db
}
...
func (p *Peers) SignalPeerConnections() {
	...
	attemptSync := func() (tryAgain bool) {
		for i := range p.Connections {
			...
			log.Println("Connection", p.Connections[i])
			log.Println("User", p.Connections[i].User)
			if err = p.Connections[i].Websocket.WriteJSON(&websocketMessage{
				Event: "offer",
				Data:  string(offerString),
				//
				// Username: p.Connections[i].Username,
				// ImageURL: p.Connections[i].ImageURL,
			}); err != nil {
				return true
			}
		}
